...
/*
            ...
            */
            Properties.Settings.Default.Reload();//moosa
#if DEBUG
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            pg.IV_Input.Initial_Potential = -((double)numericUpDown1.Value + (double)numericUpDown_set_i_off.Value); //Current + set current offset
            pg.IV_Input.Final_Potential = -((double)numericUpDown1.Value + (double)numericUpDown_set_i_off.Value);
        ...
        private void Pg_PG_EVT_AIVDataReceived(object sender, AIVEventArgs e)
        {
            if (!isOffsetRemovalProc)
            {
                ...
                double ReadVoltageOffset = (double) Properties.Settings.Default.get_v_offset;//Convert.ToDouble(tinSampleSettings1.Tables["Coefficients"].Rows[0]["ReadVoltageOffset"]);
                double ReadVoltageGain = (double)Properties.Settings.Default.get_v_gain;// Convert.ToDouble(tinSampleSettings1.Tables["Coefficients"].Rows[0]["ReadVoltageGain"]);
                double ReadCurrentOffset = (double)Properties.Settings.Default.get_i_offset;//Convert.ToDouble(tinSampleSettings1.Tables["Coefficients"].Rows[0]["ReadCurrentOffset"]);
                double ReadCurrentGain = (double)Properties.Settings.Default.get_i_gain; //Convert.ToDouble(tinSampleSettings1.Tables["Coefficients"].Rows[0]["ReadCurrentGain"]);
            ...
            if (e.Process == "AIV")
            {
                ...
                else
                {
                    ...
                    if (OffsetRemovalProc_CurrentSet < 0) //I mean -100. Now we can calculate the offset and gain
                    {
                        ...
                        numericUpDown_get_v_off.Value = Convert.ToDecimal(offsetV);
                        numericUpDown_get_v_gain.Value = Convert.ToDecimal(gainV * 100);
                        numericUpDown_get_i_off.Value = Convert.ToDecimal(offsetI * 1000);
                        numericUpDown_get_i_gain.Value = Convert.ToDecimal(gainI * 100);
                        Properties.Settings.Default.Save();//moosa
                        /*
        ...
        {
           // tinSampleSettings1.Tables["Coefficients"].Rows[0]["SetCurrentOffset"] = numericUpDown_set_i_off.Value;
          //  tinSampleSettings1.WriteXml(TinSampleSettingsxml);
        }
